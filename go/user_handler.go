...
func iconConnectDB(logger echo.Logger) (*sqlx.DB, error) {
	const (
	...
	conf.Net = "tcp"
	conf.Addr = net.JoinHostPort("192.168.1.12", "3306")
	conf.User = "isucon"
	conf.Passwd = "isucon"
	conf.DBName = "isupipe"
	conf.ParseTime = true
...
func getIconHandler(c echo.Context) error {
	...
	conn, err := iconConnectDB(c.Logger())
	dbConn = conn
	...
	var image []byte
	if err := tx.GetContext(ctx, &image, "SELECT image FROM icons WHERE user_id = ? ORDER BY id DESC LIMIT 1", user.ID); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
